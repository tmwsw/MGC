...
st.write(
    "Это приложение предоставляет рекомендации по количеству недель для закупки на основе выбранной даты."
)
...
def calculate_weeks(price, predicted_prices):
    ...
    if avg_change > 0:  # При росте цен
        if trend_strength > 5 or volatility > 8:
            return (
                1,
                f"🚨 Срочная минимальная закупка (резкий рост до +{max_change:.1f}%)",
            )
        elif trend_strength > 4:
            return 2, f"⚠️ Уменьшенная закупка (сильный рост +{avg_change:.1f}%)"
        elif trend_strength > 3:
            return 3, f"🔹 Средняя закупка (заметный рост +{avg_change:.1f}%)"
        elif trend_strength > 2:
            return 4, f"Стандартная закупка (умеренный рост +{avg_change:.1f}%)"
        elif trend_strength > 1:
            return 5, f"📉 Увеличенная закупка (незначительный рост +{avg_change:.1f}%)"
        else:
            return 6, f"💰 Максимальная закупка (стабильные цены или слабый рост)"
    else:  # При падении цен
        if trend_strength > 5 or volatility > 8:
            return 6, f"💰 Максимальная закупка (резкое падение {min_change:.1f}%)"
        elif trend_strength > 4:
            return 5, f"📈 Увеличенная закупка (сильное падение {avg_change:.1f}%)"
        elif trend_strength > 3:
            return 4, f"🔹 Средняя закупка (заметное падение {avg_change:.1f}%)"
        elif trend_strength > 2:
            return 3, f"Стандартная закупка (умеренное падение {avg_change:.1f}%)"
        elif trend_strength > 1:
            return (
                2,
                f"⚠️ Уменьшенная закупка (незначительное падение {avg_change:.1f}%)",
            )
        else:
            return (
                1,
                f"🚨 Срочная минимальная закупка (стабильные цены или слабое падение)",
            )
...
def create_features(current_price, days_ahead, date):
    features = {
        ...
        "price_change_7d": (current_price - historical_prices["Price"].iloc[-7])
        / historical_prices["Price"].iloc[-7]
        * 100,
    }
...
with col2:
    if "calculate" in st.session_state and st.session_state["calculate"]:
        ...
        future_dates = [
            date_input + timedelta(weeks=i) for i in range(0, 6)
        ]  # От выбранной даты до +5 недель (всего 6 точек)
        future_prices = []
        ...
        st.dataframe(
            forecast_df.style.format(
                {"Прогнозируемая цена": "{:,.2f}", "Изменение, %": "{:.1f}%"}
        ...
        st.info(
            "Выберите дату и нажмите кнопку 'Получить рекомендацию' для отображения результатов"
        )
...
1. **Выберите дату** в левом блоке. Это день, на который хотите получить прогноз.
2. **Нажмите кнопку "Получить рекомендацию"**.
3. Приложение проанализирует исторические данные и спрогнозирует цены на 6 недель вперед.
4. В правом блоке вы увидите:
   - 📈 График изменения цен.
   - 🔢 Оптимальное количество недель для закупки.
   - 📝 Подробное текстовое объяснение рекомендации.
5. Вы можете **скачать Excel-отчет** с детальным прогнозом.
"""
)
